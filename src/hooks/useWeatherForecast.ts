...
function processWeatherData(list: any[], startDate: string): WeatherDay[] {
  ...
  const targetDates = [
    ...
    formatISO(addDays(start, 3), { representation: 'date' }),
    formatISO(addDays(start, 4), { representation: 'date' }),
  ]
  ...
  list.forEach((item) => {
    ...
    if (targetDates.includes(dateKey)) {
      if (!dailyData[dateKey]) {
        ...
        dailyData[dateKey].high = Math.max(
          dailyData[dateKey].high,
          item.main.temp_max,
        )
        dailyData[dateKey].low = Math.min(
          dailyData[dateKey].low,
          item.main.temp_min,
        )
  ...
  return targetDates.map((date) => {
    ...
    const conditionCounts = dayData.conditions.reduce(
      (acc, condition) => {
